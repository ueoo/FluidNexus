...
if "smoke" == run_id:
    ...
    raise ValueError(f"Unknown post fix: {run_id}")

project_root = "/path/to/FluidNexusRoot"
cogvxlora_root = f"{project_root}/cogvideox_lora_outputs"
gen_pred_root = f"{cogvxlora_root}/5b_lora_all_sdedit_pi2v_future_{run_id}"
...
def one_process(view_strength):
    ...
    cur_dataset_sub_folder = f"{camera_prefix}{view_id:02d}_cogvxlora5b_future_prefix{prefix_frame_nums}_i2v3_strength{strength_str}_start{start_frame_idx}_{run_id}"
