...
def main_demo(project_root="/path/to/FluidNexusRoot", tgt_cam=0, cuda_idx=0, finetune_step=80000):
    src_cam = 2
    ...
    print(msg)
    ckpt_root = f"{project_root}/zero123_finetune_logs"
    ...
    zero_root = f"{project_root}/{dataset_name}/zero123_dataset"

    ckpt_dir = "2024-10-28T18-26-21_fluid_nexus_ball"
    config = "configs/fluid_nexus_ball.yaml"
    ...
    for frame_id in (pbr := trange(num_frames, leave=False)):
        ...
        cond_img_path = f"{zero_root}/frame_{frame_id:03d}/{src_cam:02d}.png"
        gt_img_path = f"{zero_root}/frame_{frame_id:03d}/{tgt_cam:02d}.png"
        ...
        cond_cam_path = f"{zero_root}/camera/{src_cam:02d}.npy"
        target_cam_path = f"{zero_root}/camera/{tgt_cam:02d}.npy"
        cond_RT = np.load(cond_cam_path)
        target_RT = np.load(target_cam_path)

        d_T = get_T(target_RT, cond_RT)

        raw_im = cv2.imread(cond_img_path)

        gt_im = cv2.imread(gt_img_path)
