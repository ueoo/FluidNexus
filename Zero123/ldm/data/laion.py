...
class DataWithWings(torch.utils.data.IterableDataset):
    def __init__(self, min_size, transform=None, target_transform=None):
        ...
        self.kv = OnDiskKV(file="/home/ubuntu/laion5B-watermark-safety-ordered", key_format="q", value_format="ee")  # type: ignore
        self.kv_aesthetic = OnDiskKV(file="/home/ubuntu/laion5B-aesthetic-tags-kv", key_format="q", value_format="e")  # type: ignore
    ...
    def _compute_hash(url, text):
        ...
        return mmh3.hash64(total)[0]  # type: ignore
    ...
    def make_loader(self, dataset_config, train=True):
        ...
        image_transforms.extend(
            [
                ...
                torchvision.transforms.Lambda(lambda x: rearrange(x * 2.0 - 1.0, "c h w -> h w c")),
            ]
        )
        ...
        transform_dict = {
            dkey: (
                load_partial_from_config(transforms_config[dkey])  # type: ignore
                if transforms_config[dkey] != "identity"
                else identity  # type: ignore
            )
...
def example04():
    ...
    for i_shard in range(60208)[::-1]:
        print(i_shard)
        tars = (
            "pipe:aws s3 cp s3://s-laion/improved-aesthetics-laion-2B-en-subsets/aesthetics_tars/{:06}.tar -".format(
                i_shard
            )
        )
        dataset = wds.WebDataset(tars)
