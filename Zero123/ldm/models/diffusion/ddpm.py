...
class LatentDiffusion(DDPM):
    ...
    def _rescale_annotations(self, bboxes, crop_coordinates):  # TODO: move to dataset
        def rescale_bbox(bbox):
            x0 = clamp((bbox[0] - crop_coordinates[0]) / crop_coordinates[2])  # type: ignore
            y0 = clamp((bbox[1] - crop_coordinates[1]) / crop_coordinates[3])  # type: ignore
    ...
    def p_sample(
        ...
        if return_codebook_ids:
            return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise, logits.argmax(dim=1)  # type: ignore
        if return_x0:
...
class LatentInpaintDiffusion(LatentDiffusion):
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log["masked_image"] = (
            rearrange(batch["masked_image"], "b h w c -> b c h w").to(memory_format=torch.contiguous_format).float()
        )
        return log
